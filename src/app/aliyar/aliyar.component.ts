...
@Component({
  ...
  templateUrl: './aliyar.component.html',
  styleUrl: '../styles.css'
})


export class AliyarComponent implements OnInit {
  ...
  initDataEntry(){
    this.showForm = true;
    this.showSettings = false;
    this.frl = staticData.aliyarData.frl;
    this.grossStorage = staticData.aliyarData.grossStorage;
    this.warningLevel = staticData.aliyarData.warningLevel;
    ...
    var url = `${this.url}?action=getLatestStorage&sheetName=Aliyar`;
    fetch(url, {
      method : 'GET',
    })
  ...
  sendData(){
    ...
    this.totalOutflow =this.spillway + this.sluice; 
  ...
  viewData(){
    ...
    const url = `${this.url}?action=getData&sheetName=Aliyar`;
    fetch(url, {
      method : 'GET',
    })
    .then(response => response.text())
