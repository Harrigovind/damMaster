...
@Component({
  selector: 'app-chulliyar',
  standalone: true,
  imports: [FormsModule, CommonModule],
  templateUrl: './chulliyar.component.html',
  styleUrl: '../styles.css'
})

export class ChulliyarComponent implements OnInit{
  ...
  initDataEntry(){
    this.showForm = true;
    this.showSettings = false;
    this.frl = staticData.chulliyarData.frl;
    this.grossStorage = staticData.chulliyarData.grossStorage;
    ...
    var url = `${this.url}?action=getLatestStorage&sheetName=Chulliyar`;
    fetch(url, {
      method : 'GET',
    })
  ...
  updateData(){
    ...
    this.isEntering = true;
    const percent = this.storage/this.grossStorage * 100;
    this.totalOutflow = this.spillway + this.otherOutflow + this.rbc + this.lbc + this.powerhouse;
  ...
  viewData(){
    ...
    const url = `${this.url}?action=getData&sheetName=Chulliyar`;
    fetch(url, {
      method : 'GET',
    })
    .then(response => response.text())
