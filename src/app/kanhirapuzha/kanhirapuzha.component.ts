...
@Component({
  selector: 'app-kanhirapuzha',
  standalone: true,
  imports: [FormsModule, CommonModule],
  templateUrl: './kanhirapuzha.component.html',
  styleUrl: '../styles.css'
})

export class KanhirapuzhaComponent implements OnInit{
  ...
  initDataEntry(){
    this.showForm = true;
    this.showSettings = false;
    this.frl = staticData.kanhirapuzhaData.frl;
    this.grossStorage = staticData.kanhirapuzhaData.grossStorage;
    ...
    var url = `${this.url}?action=getLatestStorage&sheetName=Kanhirapuzha`;
    fetch(url, {
      method : 'GET',
    })
  ...
  sendData(){
    ...
    this.totalOutflow =this.spillway + this.lbc + this.rbc + this.otherOutflow + this.sluice ; 
  ...
  updateData(){
    ...
    this.isEntering = true;
    const percent = this.storage/this.grossStorage * 100;
    this.totalOutflow = this.spillway + this.sluice + this.otherOutflow + this.rbc + this.lbc;
  ...
  viewData(){
    ...
    const url = `${this.url}?action=getData&sheetName=Kanhirapuzha`;
    fetch(url, {
      method : 'GET',
    })
    .then(response => response.text())
