...
@Component({
  selector: 'app-mangalam',
  standalone: true,
  imports: [FormsModule, CommonModule],
  templateUrl: './mangalam.component.html',
  styleUrl: '../styles.css'
})

export class MangalamComponent implements OnInit {
  ...
  initDataEntry(){
    this.showForm = true;
    this.showSettings = false;
    this.frl = staticData.mangalamData.frl;
    this.grossStorage = staticData.mangalamData.grossStorage;
    ...
    var url = `${this.url}?action=getLatestStorage&sheetName=Mangalam`;
    fetch(url, {
      method : 'GET',
    })
  ...
  sendData(){
    ...
    this.totalOutflow =this.spillway + this.lbc + this.rbc + this.otherOutflow + this.sluice + this.powerhouse; 
  ...
  updateData(){
    ...
    this.isEntering = true;
    const percent = this.storage/this.grossStorage * 100;
    this.totalOutflow = this.spillway + this.sluice + this.otherOutflow + this.rbc + this.lbc + this.powerhouse;
  ...
  viewData(){
    ...
    const url = `${this.url}?action=getData&sheetName=Mangalam`;
    fetch(url, {
      method : 'GET',
    })
    .then(response => response.text())
