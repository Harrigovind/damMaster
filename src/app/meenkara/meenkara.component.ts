...
@Component({
  selector: 'app-meenkara',
  standalone: true,
  imports: [FormsModule, CommonModule],
  templateUrl: './meenkara.component.html',
  styleUrl: '../styles.css'
})

export class MeenkaraComponent implements OnInit{
  ...
  initDataEntry(){
    this.showForm = true;
    this.showSettings = false;
    this.frl = staticData.meenkaraData.frl;
    this.grossStorage = staticData.meenkaraData.grossStorage;
    ...
    var url = `${this.url}?action=getLatestStorage&sheetName=Meenkara`;
    fetch(url, {
      method : 'GET',
    })
  ...
  sendData(){
    ...
    this.totalOutflow =this.spillway + this.lbc + this.rbc + this.otherOutflow + this.powerhouse; 
    this.totalInflow = this.storage - this.lastStorage + this.totalOutflow;

    if(this.totalInflow<0){
      this.totalInflow = 0;
    }
    if(!this.level || !this.storage || !this.date){
  ...
  updateData(){
    ...
    this.isEntering = true;
    const percent = this.storage/this.grossStorage * 100;
    this.totalOutflow = this.spillway + this.powerhouse + this.otherOutflow + this.rbc + this.lbc;
  ...
  viewData(){
    ...
    const url = `${this.url}?action=getData&sheetName=Meenkara`;
    fetch(url, {
      method : 'GET',
    })
    .then(response => response.text())
